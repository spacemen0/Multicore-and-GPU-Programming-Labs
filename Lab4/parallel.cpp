...
void mandelbrotThread(int start_x, int end_x, unsigned char *ptr)
{
...
void computeFractal(unsigned char *ptr)
{
    ...
    {
        int start_x = i * (gImageWidth / num_threads);
        int end_x = (i + 1) * (gImageWidth / num_threads);

        threads.emplace_back(mandelbrotThread, start_x, end_x, ptr);
    }
